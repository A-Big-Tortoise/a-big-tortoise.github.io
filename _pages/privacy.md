...
Umami stores pseudonymous, cookie-free event data including page views, referrers, page titles and URL paths, browser and operating-system categories, device type, screen dimensions, language, approximate location, and visit metrics. It associates events within a session using a rotating hash derived from data including the visitor's IP address and user agent, the hostname, and this website's ID. The raw IP address is also used to derive approximate location but is not stored, and the session is not linked to a name or email address.
...
- does not send analytics events when the browser's Do Not Track setting is enabled;
- does not assign a custom visitor identifier; and
